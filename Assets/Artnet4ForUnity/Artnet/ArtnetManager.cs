...
namespace ArtnetForUnity
{
    ...
    public class ArtnetManager : IDisposable
    {
        ...
        public ArtnetManager()
        {
            ...
            //new Thread(ListenerThread) { IsBackground = true }.Start(); // do we need a listener? (NO) its buggy (in editor)
        }
        ...
        public void SetArtnetData(int UnityUniverseNumber, byte[] UniverseData)
        {
            try
            {
                if (settings.artnetOutputs[UnityUniverseNumber] == null)
                {
                    UnityEngine.Debug.LogError("[Artnet4Unity] DMX Universe Does Not Exist. Have you added a universe via general settings?");
                }
            ...
            {
                UnityEngine.Debug.LogError("[Artnet4Unity] DMX Universe Does Not Exist. Have you added a universe via general settings? | "+ e.Message);
            }
        }

        // *********************************************************************************************
        // retreive available data for feedback 
        
        /*public int getNumberOfAvailableOutputs()
        {
            int valueToReturn = settings.artnetOutputs.Count;
        }

        public int getOutputUniverse(int whichOutput)
        {
            if (whichOutput < 0 || whichOutput >= settings.artnetOutputs.Count)
            {
                return -1;
            }
            return settings.artnetOutputs[whichOutput].Universe;
        }
        
        public string getOutputIp(int whichOutput)
        {
            if (whichOutput < 0 || whichOutput >= settings.artnetOutputs.Count)
            {
                return "-1";
            ...
            return settings.artnetOutputs[whichOutput].NodeRevcIPAddress[0];
        }*/

        public ArtnetSettings retreiveArtNetSettings()
        {
            return settings;
        }
        ...
        private void ListenerThread()
        {
            ...
            while (!ListenQueue.IsCompleted) // getting error system disposed (whic
            {
                ...
                if (udpRevcClient.Available > 0)
                {
                    ...
                    if (ListenQueue != null)
                    {
                        if (isArtnetActive) // trying this (to fix crash on exit)
                        {
                            ListenQueue.Add(pkt); // TODO crashes here when a listener is added to the (local) network, crashes when app closes 
                            RecvCallBack?.Invoke();
                        }
                    }
                }
...
// 250501 TODO commented out to prevent laptop from freaking out while in editor:
    /*
    #if UNITY_EDITOR
    [InitializeOnLoad]
