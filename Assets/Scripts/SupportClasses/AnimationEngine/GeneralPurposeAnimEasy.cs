...
{
    public class GeneralPurposeAnimEasy
    {
        ...
        private float[] deltaCutoffPosn = new float[3];
        private bool[] isDescendingPosn = new bool[3];
        ...
        private AEEaseInOutQuintOneStage[] targetPosnAnim = new AEEaseInOutQuintOneStage[3]; // x, y, and z
        ...
        private float[] deltaCutoffRotn = new float[3];
        private bool[] isDescendingRotn = new bool[3];
        ...
        private float deltaCutoffScale;
        private bool isDescendingScale = false;
        private bool scaleAnimationActive = false;
        private int scaleAnimationFrame = 0;
        private int scaleAnimationLength = 20;
        private AEEaseInOutQuintOneStage targetScaleAnim;

        // ********************************************************************************
        // Transparency
        public float alpha = -1f;
        public float targetAlpha = -1f;
        private float deltaCutoffAlpha = 0f;
        private bool isDescendingAlpha = false;
        private bool alphaAnimationActive = false;
        private int alphaAnimationFrame = 0;
        private int alphaAnimationLength = 20;
        private AEEaseInOutQuintOneStage targetAlphaAnim;

        // ********************************************************************************
        // other animated value
        public float value;
        public float targetValue;
        private float deltaCutoffValue;
        private bool isDescendingValue = false;
        private bool valueAnimationActive = false;
        private int valueAnimationFrame = 0;
        private int valueAnimationLength = 20;
        private AEEaseInOutQuintOneStage targetValueAnim;
        
        public GeneralPurposeAnimEasy()
        {
            ...
            for (i = 0; i < 3; ++i)
            {
                ...
                posn[i] = 0.0f;
                targetPosn[i] = 0.0f;
                targetPosnAnim[i] = new AEEaseInOutQuintOneStage();
            ...
            targetScaleAnim = new AEEaseInOutQuintOneStage();
            targetAlphaAnim = new AEEaseInOutQuintOneStage();
            targetValueAnim = new AEEaseInOutQuintOneStage();
        ...
        public void initAnim()
        {
            ...
            Vector3 from_v = new Vector3(10, 10, 10);
            Vector3 to_v = new Vector3(5, 5, 5);
            currentPosition = from_v;

            for (i = 0; i < 3; ++i)
            {
                posn[i] = from;
                targetPosn[i] = to;
                SetNewPosn(i, end, animLength);
            }

            currentRotation = from_v;
            
            for (i = 0; i < 3; ++i)
            {
                rotn[i] = from;
                targetRotn[i] = to;
                SetNewRotn(i, end, animLength);
            }

            scale = from;
            targetScale = to;
            SetNewScale(1, animLength);

            alpha = from;
            targetAlpha = to;
            SetNewAlpha(end, animLength);

            value = from;
            targetValue = to;
            SetNewValue(end, animLength);
        ...
        public void Update()
        {
            ...
            //float animationLengthBuffer = 1.15f;
            bool completedAnim = false;
            //unsafe
            //{
                for (int i = 0; i < 3; ++i)
                {
                    if (posnAnimationActive[i])
                    {
                        ...
                        posn[i] += delta / 2.0f;
                        completedAnim = false;
                        if (isDescendingPosn[i])
                        {
                            if (posn[i] < deltaCutoffPosn[i]) // we've completed 99%
                                completedAnim = true;
                        }
                        else
                        {
                    ...
                    if (rotnAnimationActive[i])
                    {
                        ...
                        rotn[i] += delta / 2.0f;
                        completedAnim = false;
                        if (isDescendingRotn[i])
                        {
                            if (rotn[i] < deltaCutoffRotn[i]) // we've completed 99%
                                completedAnim = true;
                        }
                        else
                        {
                ...
                if (scaleAnimationActive)
                {
                    ...
                    scale += delta / 4.0f;
                    completedAnim = false;
                    if (isDescendingScale)
                    {
                        if (scale < deltaCutoffScale) // we've completed 99%
                            completedAnim = true;
                    }
                    else
                    {
                ...
                if (alphaAnimationActive)
                {
                    ...
                    alpha += delta / 4.0f;
                    completedAnim = false;
                    if (isDescendingAlpha)
                    {
                        if (alpha < deltaCutoffAlpha) // we've completed 99%
                            completedAnim = true;
                    }
                    else
                    {
                ...
                if (valueAnimationActive)
                {
                    ...
                    value += delta / 4.0f;
                    completedAnim = false;
                    if (isDescendingValue)
                    {
                        if (value < deltaCutoffValue) // we've completed 99%
                            completedAnim = true;
                    }
                    else
                    {
        ...
        private int StartPositionAnimation(int whichAxis)
        {
            ...
            float delta = targetPosn[whichAxis] - posn[whichAxis];
            if (delta < 0)
                isDescendingPosn[whichAxis] = true;
            else
                isDescendingPosn[whichAxis] = false;
            deltaCutoffPosn[whichAxis] = targetPosn[whichAxis] - (delta * 0.01f);
        ...
        private int StartRotationAnimation(int whichAxis)
        {
            ...
            float delta = targetRotn[whichAxis] - rotn[whichAxis];
            if (delta < 0)
                isDescendingRotn[whichAxis] = true;
            else
                isDescendingRotn[whichAxis] = false;
            deltaCutoffRotn[whichAxis] = targetRotn[whichAxis] - (delta * 0.01f);
        ...
        private void StartScaleAnimation(int whichNumberOfFrames)
        {
            ...
            float delta = targetScale - scale;
            if (delta < 0)
                isDescendingScale = true;
            else
                isDescendingScale = false;
            deltaCutoffScale = targetScale - (delta * 0.01f);
        ...
        private void StartAlphaAnimation(int whichNumberOfFrames)
        {
            ...
            if (delta < 0)
                isDescendingAlpha = true;
            else
                isDescendingAlpha = false;

            deltaCutoffAlpha = targetAlpha - (delta * 0.01f);
        ...
        private void StartValueAnimation(int whichNumberOfFrames)
        {
            ...
            if (delta < 0)
                isDescendingValue = true;
            else
                isDescendingValue = false;

            deltaCutoffValue = targetValue - (delta * 0.01f);
