...
namespace MarnoldSacn.Layers.Framing
{
	public class DataPacketFramingLayer : FramingLayer
	{
		...
		private byte priority = MarnoldSacnConstants.Priority_Default;
		public byte Priority
		{
			get { return priority; }
			set
			{
				if (value > MarnoldSacnConstants.Priority_MaxValue)
				{
					throw new ArgumentOutOfRangeException($"Priority may not exceed {MarnoldSacnConstants.Priority_MaxValue}");
				}
				priority = value;
			}
		}
		...
		public UInt16 Universe
		{
			...
			set
			{
				if (value < MarnoldSacnConstants.Universe_MinValue || value > MarnoldSacnConstants.Universe_MaxValue)
				{
					throw new ArgumentOutOfRangeException($"Universe must be between {MarnoldSacnConstants.Universe_MinValue} and {MarnoldSacnConstants.Universe_MaxValue}");
				}
				universe = value;
			}
		}
