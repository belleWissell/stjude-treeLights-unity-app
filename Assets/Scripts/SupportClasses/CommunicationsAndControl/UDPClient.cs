...
{
    // ADOPTED FROM https://forum.unity.com/threads/simple-udp-implementation-send-read-via-mono-c.15900/
    public class UdpConnection
    {
        ...
        public void StartConnection(string sendIp, int sendPort, int receivePort)
        {
            ...
            {
                Debug.Log("[UDPCLILENT] Failed to listen for UDP at port " + receivePort + ": " + e.Message);
                return;
            }

            Debug.Log("[UDPCLILENT] Created receiving client at ip  and port " + receivePort);
            this.senderIp = sendIp;
            this.senderPort = sendPort;

            Debug.Log("[UDPCLILENT] Set sendee at ip " + sendIp + " and port " + sendPort);
        ...
        private void ListenForMessages(UdpClient client)
        {
            ...
            while (threadRunning)
            {
                try
                {
                    ...
                    //Byte[] receiveBytes = client.Receive()
